...
contract GameTokenUnitTest is Test {
    ...
    function setUp() public {
        token = new GameToken(admin, admin, 1000e18);
    }
    ...
    function test_mint_revertsAtCap() public {
        vm.prank(admin);
        vm.expectRevert(
            abi.encodeWithSelector(GameToken.CapExceeded.selector, 100_000_001e18, 100_000_000e18)
        );
    ...
    function test_delegate_grantsVotingPower() public {
        ...
        assertEq(token.getVotes(admin), 1000e18);
    }

    function test_permit_works() public {
        // EIP-2612 permit happy path
        uint256 ownerKey = 0xA11CEBEEF;
    ...
    function test_pastVotes() public {
        ...
        vm.warp(ts + 1);
        assertEq(token.getPastVotes(admin, ts - 1), 1000e18);
    }
}
